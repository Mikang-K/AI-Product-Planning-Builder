(function () {
  ...
  function loadState() {
    try {
      ...
      if (raw) {
        ...
        state.activeTab = parsed.activeTab || "diagnosis";
        const { apiKey, ...persistedLlmConfig } = parsed.llmConfig || {};
        state.llmConfig = {
          ...state.llmConfig,
          ...persistedLlmConfig,
          apiKey: "",
      ...
      <section class="panel full collaboration-board">
        <div class="collaboration-head">
          ...
          <div class="collaboration-actions">
            <button id="exportCollaborationButton" class="secondary-button" type="button">Full JSON</button>
            <button id="exportDeveloperPackageButton" class="secondary-button" type="button">Dev JSON</button>
            <button id="exportValidatorPackageButton" class="secondary-button" type="button">Validation JSON</button>
            <button id="importAgentResultButton" class="secondary-button" type="button">Result Import</button>
  ...
  function renderWorkItemCard(item) {
    return `
      <article class="work-item-card">
        ...
        <button class="secondary-button" type="button" data-work-item-export-id="${escapeHtml(item.id)}">Export Item</button>
      </article>
    `;
  }
  ...
  function normalizeAgentResult(result, collaboration) {
    ...
    return {
      ...
      agentRole,
      status,
      findings: normalizeStringArray(result.findings, [], 0),
      recommendedChanges: normalizeStringArray(result.recommendedChanges, [], 0),
      changedFiles: normalizeStringArray(result.changedFiles, [], 0),
      tests: normalizeStringArray(result.tests, [], 0),
      risks: normalizeStringArray(result.risks, [], 0),
  ...
  function exportActivePrd() {
    const project = getActiveProject();
    if (!project) return;
    const blob = new Blob([project.artifacts.prd], {
      type: "text/markdown",
    });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `${slugify(project.title)}-prd.md`;
    anchor.click();
    URL.revokeObjectURL(url);
  }
  ...
  function ensureProjectCollaboration(project) {
  ...
  function buildCollaborationPackage(project) {
    const collaboration = ensureProjectCollaboration(project);
    const artifacts = project.artifacts;
    return {
  ...
  function buildDevelopmentWorkItems(project) {
    const artifacts = project.artifacts || {};
  ...
  function buildValidationWorkItems(project) {
  ...
  function mergeGeneratedWorkItems(existingItems, generatedItems) {
  ...
  function normalizeImportedProjects(payload) {
    ...
    const importedProjects = candidates.map((project) => normalizeImportedProject(project, existingIds));
  ...
  function normalizeImportedProject(project, existingIds) {
    ...
    migrateProjectArtifacts(imported);
    return imported;
  }
...
리스크, MVP 적정성, 실험 설계 타당성, 출시 전 체크리스트를 평가하세요.`,
  ...
  function developmentPackageSchema() {
    return {
      type: "object",
  ...
  function validationPackageSchema() {
    return {
      type: "object",
...
- 세 에이전트가 피드백 루프를 형성해 최종 Product Package를 자동 완성한다.`;
  ...
  function prioritizeMetric(metrics, keyword) {
